...
int main(int argc,char**argv){
  try{
    ...
    auto*renderer=SDL_CreateSoftwareRenderer(surface);require(renderer,"renderer unavailable");
    {
      ...
      shell.acceptanceSkipBoot();
      auto finishDeviceLock=[&]{
        const auto deadline=SDL_GetTicks()+15000;
        while(shell.deviceLock().busy()&&SDL_GetTicks()<deadline){SDL_Delay(5);shell.update(.005);}
        require(!shell.deviceLock().busy(),"device lock timed out");
      };
      finishDeviceLock();
      require(!shell.acceptanceLaunch("finance"),"Finance bypassed device PIN");
      ...
      require(shell.acceptanceLaunch("finance"),"Finance unavailable");
