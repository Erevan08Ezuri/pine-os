...
#include <cmath>
namespace Pine {
void CameraApp::render(UiContext&ui){ui.renderCamera();}
  ...
  if(camera_.available()&&camera_.previewFrame(frame)&&frame.width>0&&frame.height>0&&!frame.pixels.empty()){
    ...
    if(surface){
      ...
      if(texture){
        ...
        const float maxW=preview.w-30,maxH=preview.h-30;
        const float scale=std::min(maxW/static_cast<float>(frame.width),maxH/static_cast<float>(frame.height));
        const float w=frame.width*scale,h=frame.height*scale;
        SDL_FRect dst{preview.x+(preview.w-w)/2,preview.y+(preview.h-h)/2,w,h};
        live=SDL_RenderTexture(renderer_,texture,nullptr,&dst);
        SDL_DestroyTexture(texture);
      }
      SDL_DestroySurface(surface);
    }
  }
  if(!live){label(195,550,"WAITING FOR CAMERA",3.2f);sublabel(170,605,camera_.lastStatus());}
#else
