...
namespace Pine {
void TextInputManager::focus(TextInputSession&session,std::uint64_t now,bool requestSoftware){auto* previous=focused_;focused_=&session;targetVisible_=requestSoftware&&(forceSoftware_||!physicalKeyboardActive(now));if(previous&&previous!=&session)previous->notifyBlurred();}
void TextInputManager::blur(){auto* previous=focused_;focused_=nullptr;targetVisible_=false;if(previous)previous->notifyBlurred();}
...
void TextInputManager::hideKeyboard(bool clearFocus){targetVisible_=false;if(clearFocus)blur();}
}
