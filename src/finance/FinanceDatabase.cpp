...
FinanceDatabase::FinanceDatabase(std::filesystem::path root){try{std::filesystem::create_directories(root/"finance");path_=root/"finance"/"finance.db";const auto opened=sqlite3_open_v2(path_.string().c_str(),&db_,SQLITE_OPEN_READWRITE|SQLITE_OPEN_CREATE|SQLITE_OPEN_FULLMUTEX,nullptr);if(opened!=SQLITE_OK){const auto reason=db_?sqlite3_errmsg(db_):"storage unavailable";if(db_)sqlite3_close(db_);db_=nullptr;openMemoryFallback(reason);return;}sqlite3_busy_timeout(db_,5000);try{execute("PRAGMA foreign_keys=ON; PRAGMA journal_mode=WAL; PRAGMA synchronous=NORMAL;");migrate();}catch(const std::exception&e){sqlite3_close(db_);db_=nullptr;auto recovery=path_;recovery+=std::format(".recovery-{}",std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now().time_since_epoch()).count());std::error_code ec;std::filesystem::rename(path_,recovery,ec);if(ec){openMemoryFallback(std::string("Database corrupt; original preserved: ")+e.what());return;}warning_="Damaged database preserved as "+recovery.filename().string();check(sqlite3_open_v2(path_.string().c_str(),&db_,SQLITE_OPEN_READWRITE|SQLITE_OPEN_CREATE|SQLITE_OPEN_FULLMUTEX,nullptr),db_,"Create recovery database");sqlite3_busy_timeout(db_,5000);execute("PRAGMA foreign_keys=ON; PRAGMA journal_mode=WAL; PRAGMA synchronous=NORMAL;");migrate();Logger::instance().error("FINANCE",warning_);}}catch(const std::exception&e){if(db_)sqlite3_close(db_);db_=nullptr;openMemoryFallback(e.what());}}
