...
Money Money::operator-(const Money&o)const{if(o.minor==std::numeric_limits<std::int64_t>::min())throw std::overflow_error("Money overflow");return *this+Money{-o.minor,o.currency};}
std::optional<std::int64_t> parseMoneyMinor(const std::string&input,const Currency&currency){std::string s;for(unsigned char c:input)if(!std::isspace(c)&&c!='$'&&c!=',')s.push_back(static_cast<char>(c));if(s.empty())return{};bool neg=false;if(s.front()=='+'||s.front()=='-'){neg=s.front()=='-';s.erase(s.begin());}if(s.empty())return{};auto dot=s.find('.');if(dot!=std::string::npos&&s.find('.',dot+1)!=std::string::npos)return{};std::string whole=dot==std::string::npos?s:s.substr(0,dot),fraction=dot==std::string::npos?"":s.substr(dot+1);if(whole.empty())whole="0";if(!std::ranges::all_of(whole,[](unsigned char c){return std::isdigit(c);})||!std::ranges::all_of(fraction,[](unsigned char c){return std::isdigit(c);})||fraction.size()>static_cast<std::size_t>(currency.minorDigits))return{};while(fraction.size()<static_cast<std::size_t>(currency.minorDigits))fraction.push_back('0');try{std::int64_t scale=1;for(int i=0;i<currency.minorDigits;++i)scale*=10;const auto major=std::stoll(whole);const auto part=fraction.empty()?0:std::stoll(fraction);if(major>(std::numeric_limits<std::int64_t>::max()-part)/scale)return{};auto result=major*scale+part;return neg?-result:result;}catch(...){return{};}}
