...
bool CameraService::previewFrame(CameraFrame& frame){
    if(!available()){status_="No camera detected.";return false;}
    if(!backend_.start()){status_="Camera startup failed";Logger::instance().warn("CAMERA",status_);return false;}
    if(!backend_.previewFrame(frame))return false;
    status_="Camera live";return true;
}
