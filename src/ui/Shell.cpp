...
void Shell::render(){SDL_SetRenderDrawColor(renderer_,Theme::Background.r,Theme::Background.g,Theme::Background.b,255);SDL_RenderClear(renderer_);const bool booting=!skipBoot_&&SDL_GetTicks()-started_<2600;if(booting)renderBoot();else if(pinScreenVisible())renderPinScreen();else{renderShell();if(promptAction_)renderPrompt();keyboard_.render(*this,720,1280);}if(!pinScreenVisible()&&!toast_.empty()&&SDL_GetTicks()<toastUntil_){const float y=std::min(1110.0f,textInput_.safeContentBottom(1280)-80);panel({85,y,550,70});label(110,y+22,toast_,2.5f);}if(!capturePath_.empty()){SDL_Surface*surface=SDL_RenderReadPixels(renderer_,nullptr);captureSucceeded_=surface&&SDL_SaveBMP(surface,capturePath_.string().c_str());if(surface)SDL_DestroySurface(surface);capturePath_.clear();}SDL_RenderPresent(renderer_);click_=false;}
