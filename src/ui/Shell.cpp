...
Shell::Shell(SDL_Window*w,SDL_Renderer*r,Configuration&c,std::unique_ptr<Platform>p):window_(w),renderer_(r),config_(c),platform_(std::move(p)),battery_(platform_->battery()),camera_(platform_->camera(),c.storageRoot()),bluetooth_(platform_->bluetooth()),audio_(platform_->audio()),network_(platform_->network()),usb_(platform_->usb()),display_(platform_->display()),files_(c.storageRoot()),system_(c,*platform_),notes_(c.dataRoot()){
  ...
  apps_.registerApp(std::make_unique<FilesApp>());apps_.registerApp(std::make_unique<CameraApp>());apps_.registerApp(std::make_unique<BluetoothApp>());apps_.registerApp(std::make_unique<NotesApp>());apps_.registerApp(std::make_unique<SettingsApp>());
...
void Shell::handleEvent(const SDL_Event&e){
  ...
  if(event.type==SDL_EVENT_MOUSE_MOTION&&pointerDown_&&apps_.currentId()=="notes"&&!textInput_.keyboardVisible()){
    const float dy=event.motion.y-pointerY_;if(std::abs(dy)>1){pointerDragged_=true;if(notesView_==NotesView::List)notesScroll_=std::max(0.0f,notesScroll_-dy);else bodyScroll_=std::max(0.0f,bodyScroll_-dy/28.0f);pointerY_=event.motion.y;}
  }
  if(event.type==SDL_EVENT_MOUSE_WHEEL&&apps_.currentId()=="notes"){if(notesView_==NotesView::List)notesScroll_=std::max(0.0f,notesScroll_-event.wheel.y*55);else bodyScroll_=std::max(0.0f,bodyScroll_-event.wheel.y*2);}
...
void Shell::update(double dt){if(auto*app=apps_.current())app->update(dt);const auto now=SDL_GetTicks();textInput_.update(dt);keyboard_.update(now,textInput_);if(noteSaveDebounce_.ready(now)){commitNote();noteSaveDebounce_.clear();}if(auto error=notes_.consumeError();!error.empty())toast("NOTE NOT SAVED - WILL REMAIN IN MEMORY");const auto delta=now-lastFrame_;if(delta>0)fps_=fps_*.9+(1000.0/delta)*.1;lastFrame_=now;}
...
bool Shell::launchApp(const std::string&id){if(apps_.currentId()=="notes"&&id!="notes")commitNote();dismissInput();deleteNotePrompt_=false;return apps_.launch(id);}
void Shell::goHome(){commitNote();dismissInput();deleteNotePrompt_=false;if(notesView_==NotesView::Editor)notesView_=NotesView::List;apps_.home();}
...
void Shell::openTextPrompt(const std::string&title,std::string initial,std::function<void(const std::string&)>action){
  promptTitle_=title;promptText_=std::move(initial);promptAction_=std::move(action);
  promptSession_=std::make_unique<TextInputSession>(promptText_,InputType::Text,TextInputSession::Callback{},[this]{if(promptAction_&&!promptText_.empty()){auto action=std::move(promptAction_);auto value=promptText_;promptAction_={};dismissInput();action(value);}},TextInputSession::Callback{},64);focusInput(*promptSession_);
}
