...
if(s.button({70,y-15,555,50},s.battery().isCharging()?"CHARGING ON":"CHARGING OFF"))s.battery().simulateCharging(!s.battery().isCharging());
y+=65;
if(s.button({70,y-15,265,50},s.network().wifiEnabled()?"WIFI ON":"WIFI OFF"))s.network().setWifiEnabled(!s.network().wifiEnabled());
if(s.button({360,y-15,265,50},s.bluetooth().enabled()?"BT ON":"BT OFF"))s.bluetooth().setEnabled(!s.bluetooth().enabled());
y+=65;
if(s.button({70,y-15,555,50},"USB "+s.usb().modeName()))s.usb().cycle();
y+=65;
if(s.button({70,y-15,555,50},s.camera().available()?"CAMERA AVAILABLE":"CAMERA UNAVAILABLE"))s.camera().simulateAvailable(!s.camera().available());
y+=65;
if(s.button({70,y-15,555,50},s.textInput().forceSoftwareKeyboard()?"SOFT KEYBOARD FORCED":"SOFT KEYBOARD AUTO"))s.textInput().setForceSoftwareKeyboard(!s.textInput().forceSoftwareKeyboard());}
}
