...
struct BluetoothDevice {
    ...
    int rssi{127}; // 127 means no signal measurement is available.
    bool connectable{true},connecting{false},disconnecting{false};
};
struct BluetoothSnapshot {
    bool enabled{false},scanning{false},busy{false};
...
public:
    ...
    virtual BluetoothSnapshot snapshot()const=0;
    virtual bool enabled()const{return snapshot().enabled;}
    std::vector<BluetoothDevice> devices()const{return snapshot().devices;}
    virtual void setEnabled(bool)=0;
    virtual void startScan()=0;
    virtual void stopScan()=0;
    // True means the request was accepted; snapshot reports its actual outcome.
...
struct WifiNetwork {std::string ssid,security;int rssi{};bool connectable{true};};
struct WifiScanSnapshot {bool scanning{};std::string status;std::vector<WifiNetwork> networks;};
class NetworkBackend { public:virtual~NetworkBackend()=default;virtual bool wifiEnabled()const=0;virtual void setWifiEnabled(bool)=0;virtual bool connected()const=0;virtual std::string networkName()const=0;virtual bool configure(const std::string&,const std::string&){return false;}virtual void startScan()=0;virtual WifiScanSnapshot scanSnapshot()const=0;};
