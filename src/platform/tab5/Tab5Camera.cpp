...
bool Tab5Camera::initialize(){
    ...
    for(unsigned i=0;i<bufferCount_;++i){
        ...
        // The pinned esp_video mmap shim returns NULL on failure, not POSIX's
        // MAP_FAILED sentinel (which its header does not define).
        if(!mapped){release();failed_=true;return false;}
