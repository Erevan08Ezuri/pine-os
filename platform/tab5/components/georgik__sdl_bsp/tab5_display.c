...
static atomic_uint_fast32_t refresh_epoch;
static bool sitronix;
static bool signal_semaphore(SemaphoreHandle_t semaphore) {
    if(!semaphore)return false;
    // Color-copy completion can be invoked synchronously, while display DMA
    // callbacks arrive from ISR context. Match the FreeRTOS API accordingly.
...
static bool refresh_finished(esp_lcd_panel_handle_t panel,esp_lcd_dpi_panel_event_data_t* event,void* ctx) {
    atomic_fetch_add_explicit(&refresh_epoch,1,memory_order_relaxed);
    return false;
}
...
esp_err_t pine_tab5_present(const void* pixels) {
    ...
    if(direct){
        // Snapshot *after* selecting the new framebuffer. If a refresh races
        // between draw_bitmap and this load, we simply wait one extra refresh,
        // which is safe. We can never accidentally treat an old refresh token
        // as acknowledgement of the new front buffer.
        const uint_fast32_t observed=atomic_load_explicit(&refresh_epoch,memory_order_relaxed);
        const TickType_t start=xTaskGetTickCount();
        while(atomic_load_explicit(&refresh_epoch,memory_order_relaxed)==observed){
            if(xTaskGetTickCount()-start>pdMS_TO_TICKS(100))return ESP_ERR_TIMEOUT;
